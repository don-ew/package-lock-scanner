...
const FileUpload: React.FC<FileUploadProps> = ({ onFileSelect, file, onClear }) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      if(files[0].name === 'package-lock.json') {
          onFileSelect(files[0]);
      } else {
        alert("Please upload a file named 'package-lock.json'");
      }
    }
  ...
  return (
    <div>
        <h2 className="text-xl sm:text-2xl font-bold text-slate-100">Upload your file</h2>
        <p className="text-slate-400 mt-2">To begin, upload or drag and drop your `package-lock.json` file.</p>
        ...
        {!file ? (
            ...
            >
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-slate-500">package-lock.json</p>
                </div>
